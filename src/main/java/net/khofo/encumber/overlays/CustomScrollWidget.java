...
/*public class CustomScrollWidget extends AbstractScrollWidget {
    ...
    protected int getInnerHeight() {
        int height = 0;
        for (DropdownMenu menu : dropdownMenus) {
            height += menu.calculateHeight(this.getX(), height, 20);
        }
        return height;
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        for (DropdownMenu menu : dropdownMenus) {
            if (menu.mouseClicked(mouseX, mouseY, button, this.getX(), y, indent)) {
                return true;
            }
            y += menu.calculateHeight(this.getX(), y, indent);
        }
        return super.mouseClicked(mouseX, mouseY, button);
    }
    ...
    @Override
    public void setScrollAmount(double scrollAmount) {
        double maxScroll = Math.max(0, getInnerHeight() - this.height); // Calculate the maximum scroll amount
        this.setScrollAmount(Math.max(0, Math.min(scrollAmount, maxScroll))); // Clamp the scroll amount between 0 and maxScroll
    }
